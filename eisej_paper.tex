...
\begin{lstlisting}[caption={Implementasi Metode onCreate pada BaseActivity}, label={lst:oncreate\_smell}]
...
\begin{lstlisting}[caption={Implementasi Metode proceed pada SplashScreen}, label={lst:proceed\_smell}]
	private fun proceed() {
		postDelayed(12_000) {
...
\begin{table}[H]
	\centering
	\small % Membuat ukuran tulisan tabel sedikit lebih kecil agar rapi
	\caption{Comparison of Metric Values: Manual vs. Kopyt}
	\label{tab:manual_vs_kopyt}
	\vspace{0.2cm}
	
	% Total susunan koefisien h_size wajib berjumlah 3.0 (karena ada 3 kolom X)
	\begin{tabularx}{\textwidth}{
...
Table \ref{tab:method_metrics_manual_comparison} presents the comparison between the manually calculated values and those generated automatically by the proposed extraction pipeline.

\begin{table}[H]
	\centering
	\small
	\caption{Comparison of Metric Values Across Specific Methods}
	\label{tab:method_metrics_manual_comparison}
	\vspace{0.2cm}
	...
	\begin{tabularx}{\textwidth}{
		...
		\toprule
		\textbf{Metric} & \textbf{BaseActivity.onCreate()} & \textbf{SplashScreen.proceed()} \\
		\midrule
		ATFD & 35 & 33 \\
		FDP & 35 & 21 \\
		CM & 6 & 1 \\
...
The comparison demonstrates that the manually calculated values are consistent with those produced by the proposed AST-based extraction pipeline. For BaseActivity.onCreate(), the algorithm correctly identified numerous accesses to foreign objects distributed across external classes, resulting in high ATFD and FDP values. Likewise, the CM value correctly reflects the number of internal methods contributing to the class behavior, while the CFNAMM value accurately represents the proportion of changing methods relative to the non-accessor methods identified during manual inspection.
